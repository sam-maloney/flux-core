...
class CliJobspecSyntaxError(Exception):
    """
    Specialized SyntaxError exception to allow CliJobspecParser to throw
...
class CliJobspecLexer(object):
    """
    Simple CLI jobspec resources syntax lexical analyzer based on RFC 46.
    Used mainly as the lexer for BaseCliJobspecParser
    """
...
class CliJobspecParser:
    r"""
    Base CLI Jobspec resources syntax parser class.
    ...
    def __init__(
        ...
        super().__init__()
        self.lexer = CliJobspecLexer() if lexer is None else lexer
    ...
    def p_error(self, p):
        if p is None:
            if self.lexer.bracket_level > 0:
                ...
                raise CliJobspecSyntaxError(f"Unclosed bracket in position {pos}")
            if self.lexer.brace_level > 0:
                pos = self.lexer.last_lbrace
                raise CliJobspecSyntaxError(f"Unclosed brace in position {pos}")
            raise CliJobspecSyntaxError(f"Unexpected end of input in '{self.query}'")
        if self.lexer.bracket_level < 0:
            raise CliJobspecSyntaxError(
        ...
        raise CliJobspecSyntaxError(f"Invalid token '{p.value}' at position {p.lexpos}")
    ...
    def p_slot(self, p):
        ...
        if self.lexer.slot_count > 1:
            raise CliJobspecSyntaxError("Multiple slots require labels")
        p[0] = p[1]
...
if __name__ == "__main__":
    """
    Test command which can be run as flux python -m flux.cli_jobspec.parser
    Also used to generate ply's parsetab.py in defined outputdir.
    """

    argparser = argparse.ArgumentParser(prog="cli_jobspec.parser")
    ...
    if args.outputdir:
        print(f"Generating cli_jobspec parsetab.py in {args.outputdir}")

    parser = CliJobspecParser(
        optimize=False,
        debug=True,
        write_tables=True,
