...
jobreq_create (const flux_msg_t *msg)
{
    ...
    if (jj_range_get_counts_json (jobspec, &job->jj) < 0)
        job->errnum = errno;
...
static struct rlist *sched_alloc (struct simple_sched *ss,
                                  ...
                                  flux_error_t *errp)
{
    int nnodes = job->jj.nnodes ? count_first (job->jj.nnodes) : 0;
...
static int try_alloc (flux_t *h, struct simple_sched *ss)
{
    ...
    struct jj_range_counts *jj = NULL;
    char *R = NULL;
    struct jobreq *job = zlistx_first (ss->queue);
...
// TODO: deal with null pointers (maybe a helper function?)
// TODO: also need to free resulting strings in any case, otherwise leaking!
                            ...
                            void *arg)
{
    ...
    if (jj_range_get_counts_json (jobspec, &jj) < 0) {
        errmsg = jj.error;
        goto err;
    }
    if (jj.slot_gpus) {
    ...
    int nnodes = jj.nnodes ? count_first (jj.nnodes) : 0;
    struct rlist_alloc_info ai = {
        .mode = ss->alloc_mode,
        .nnodes = nnodes,
