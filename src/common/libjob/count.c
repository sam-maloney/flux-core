                            ...
                            json_error_t *error)
{
    ...
    // if just min given, set last_value here to avoid long loop in count_last()
    if (count->max == COUNT_MAX && count->operand == 1 && count->operator == '+') {
        count->last_value = COUNT_MAX;
    }
    count->isrange = true;
    return count;
error:
...
// TODO: do I need to free the return from json_loads?
    if (*s == '{') {
        if (!(count = count_create (json_loads (s, 0, NULL), NULL))) {
            goto error_inval;
        }
        return count;
    }
